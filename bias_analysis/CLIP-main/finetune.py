...
if __name__ == "__main__":
    ...
    # load western from command line
    if len(sys.argv) != 2:
        print("Usage: python3 finetune.py <western/nonwestern>")
        exit()
    ...
    train_dataset = RecipeDataset(df, split = train_split, device = device, data_path = DATA_PATH, image_preprocessor = preprocess, western = WESTERN, western_dict = id_to_label)
    ...
    for epoch in range(5):
        ...
        torch.save(model.state_dict(), ("western" if WESTERN else "nonwestern") + "_" + str(epoch) + "_model.pt")
        if DEBUG:
            break